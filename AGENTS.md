...
- 默认将同源 `/api` 和 `/uploads` 代理到测试服务 `https://nt.kfqzhsq.cn:8443`（必须保留端口，与小程序使用相同 HTTPS 入口）；
...
`apps/miniapp/` 已初始化为 UniApp、Vue 3、TypeScript 与 Vite 应用。首次运行前复制 `apps/miniapp/.env.example` 为不提交的 `.env.local`，并配置完整的 `VITE_API_BASE_URL`；当前测试服务为 `https://nt.kfqzhsq.cn:8443`，已有 `.env.development.local` 时也需同步更新并保留端口。
