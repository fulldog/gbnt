...
>
> 方案日期：2026-09-04；实施状态更新：2026-09-05。

## 0. 当前实施状态

截至 2026-09-05，首轮工程化实现已经落到 `apps/miniapp/`：

- 已初始化 UniApp、Vue 3、TypeScript、Vite、Pinia、SCSS、Vitest 和微信小程序构建配置；
- 已实现登录与滑块验证、会话恢复、待办、巡查上报、详情、分项整改、地图、我的清单和修改密码；
- 已接入当前 `/api/app/**` 与 `/api/attachments/images` 正式契约，不使用原型假数据；
- 已增加上报领域规则、会话持久化与列表竞态测试，并可生成 `dist/build/mp-weixin` 构建产物。

正式发布前仍须完成以下外部验收门槛：配置真实 AppID、正式 HTTPS 合法域名与法务文本，并在微信开发者工具和真机上验证定位、相机、Canvas、上传与隐私授权。第 5 节列出的后端权限和非题项整改闭环缺口仍然有效。

2026-09-05 第二轮已补充 UI、弱网恢复、数据校验与整改轮次修复，详见 [小程序 UI 与数据修复记录](miniapp-ui-data-fix-record.md)。此轮不等于生产验收通过，也未执行数据库迁移或发布。
...
当前 `apps/miniapp/` 已从业务 API 适配层扩展为可构建的 UniApp 应用；本文件后续章节继续作为实现约束和验收依据。
...
2026-09-05 按服务端实现复核后，以下问题不能由小程序隐藏按钮或客户端校验解决。当前页面可以用于工程联调，但在服务端修复并补充权限/并发测试前不能作为生产验收通过：

- **P0，数据级权限**：`/api/app/**` 跳过管理端 RBAC 后，详情仍按问题 ID 直接查询，创建只验证组织存在，整改只验证当前责任人；服务端必须显式约束登录人的组织范围、上报人或责任人关系，且附件访问不能依赖前端隐藏 URL；
- **P0，机井数量损坏闭环**：出水口或护筒损坏数量可触发 `new`，却没有对应 `QuizType`。当前前端会阻止伪造整改项；服务端必须增加正式整改模型或定义合法关闭方式，同时拒绝与当前异常项无关的整改类型；
- **P1，整改并发与轮次（代码已修复，待迁移部署和联调）**：认领检查与整改、重新整改改为同一问题行锁事务；新增当前轮次与记录轮次，只按本轮覆盖情况判断完成，双端提交 `expected_round` 拒绝旧页面写入新轮。旧服务兼容不是完整轮次保护，必须同步部署后端和双端；
- **P1，“我的”范围**：我上报应统一采用正式 `report_user_id` 语义；待整改统计及列表需要确认是否同时包含 `new` 与 `pending`；
- **P1，取证校验**：机井出水两张照片及至少 60 秒间隔若属于审计要求，必须由服务端根据可信元数据校验，不能信任客户端传入的 `mustImg`。
...
UniApp 基础工程和首版页面已经完成。下一步先处理第 5.1 节的数据权限与整改闭环阻断，并同步共享契约和正式前端 API 方法；随后配置真实 AppID、HTTPS 合法域名及法务文本，在微信开发者工具完成接口联调，最后用不同组织和责任人账号进行真机权限、定位、拍照、签名、弱网与完整整改轮次验收。
