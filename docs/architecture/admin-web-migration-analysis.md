...
> 决策日期：2026-09-04。
>
> 2026-09-05 校正：字段以后端为准，不代表可以删掉原型的信息结构。工作台须保留整改趋势与待办；街道台账和排查汇总须采用原型的多级表头、合并单元格与报表布局。具体变更与真实数据边界见 [后台原型对齐记录](admin-prototype-alignment.md)。
...
- 后端没有提供的查询、统计或编辑能力，不使用假数据模拟；原型已经明确的信息结构不能直接降级成普通列表，应补正式契约，并在未采集字段处说明缺口；
- 正式前端不得导入、修改、删除或重命名原型目录中的任何文件。

这项决策取代上一版报告中“先修改后端以对齐原型三态和题目”的建议。保留后端业务枚举，不把“字段以后端为准”解释成“原型中的趋势、待办和报表结构可以省略”。
...
| 工作台 | `/workbench` | 总量、三态数量、完成率、整改趋势、类型排行、7 列待办；趋势和待办由专用后端接口提供 | 不用上报日冒充完成日、不用假数据填补接口失败 |
| 专项整改 | `/issues` | 筛选、分页、列表、详情、新增、基础编辑、删除、整改、重新整改、重新指派、JSON 批量导入 | 原型的全量导出、三级名称联表字段 |
| 街道台账 | `/ledger/street` | 16 列、三级分组表头、年份/街道/村跨行合并、原型标题与脚注，正式报表查询和当前展示报表导出 | 未采集资产数不由问题数推导；移交编辑需另行确定持久化方案 |
| 排查汇总 | `/ledger/survey` | 22 列、四层表头、左侧 4 列固定、表内横向滚动、正式记录口径问题/整改统计 | 不由记录条数推断设施总数、正常资产数或全面排查完成 |
...
- ECharts，用于工作台整改趋势，配合类型排行及可读数据表；
