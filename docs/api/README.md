...
> 前端参考：`prototypes/static-demo/web`、`prototypes/static-demo/miniapp`（只读原型，不是正式前端）
...
目录：`apps/server/logs/{info,access,error,slow,sql}/`。
...
本地落盘：`apps/server/storage/uploads/`（配置 `upload.root`）。单张大小受 `upload.max_file_size` 限制；一次最多 20 张。
...
H5 能力参考 `prototypes/static-demo/miniapp`（登录 / 待办 / 上报 / 整改 / 我的）。附件上传走 `/api/attachments`，排查图走 `type_ext.checklist[].files`，整改走 `file_uuids`。详见 [api.md · H5 app API](./api.md)。

## 6. 配置要点

`apps/server/configs/config.yaml`：
