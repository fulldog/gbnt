  ...
  <table class="ledger-sheet ledger-sheet--street" aria-label="街道台账">
    ...
    <thead>
      ...
      <tr>
        ...
        <th rowspan="3" scope="col">新村/社区</th><th rowspan="3" scope="col">自然村</th><th :colspan="quantityFields.length" scope="colgroup">村建设项目上报情况</th>
      </tr>
      <tr>
        <th colspan="2" scope="colgroup">机井</th><th colspan="2" scope="colgroup">桥、涵、闸</th><th colspan="2" scope="colgroup">道路</th>
        <th colspan="2" scope="colgroup">独立林网</th><th colspan="2" scope="colgroup">变压器</th><th rowspan="2" scope="col">负责人签字及村委盖章</th><th rowspan="2" scope="col">电话</th>
      </tr>
      <tr>
    ...
    <tbody>
      <tr v-for="(row, index) in rows" :key="row.row_key">
        ...
        <td v-for="field in quantityFields" :key="field" :class="{ 'ledger-missing': row[field] == null }" :title="row[field] == null ? '未采集、无对应记录或数据不完整，详见统计口径' : undefined">{{ ledgerCell(row[field]) }}</td>
      </tr>
      <tr v-if="!rows.length" class="ledger-empty"><td :colspan="columnCount">{{ emptyText }}</td></tr>
    </tbody>
    <tfoot>
      <tr v-for="(note, index) in notes" :key="index" class="ledger-foot-row"><td :colspan="columnCount">统计口径：{{ note }}</td></tr>
