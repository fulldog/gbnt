...
const title = computed(() => {
  ...
  const selectedName = selectedId
    ...
    : "";
  return `聊城经济技术开发区高标准农田建设项目${selectedName || "街道"}台账`;
});
const emptyText = computed(() => loading.value || (!hasLoaded.value && !loadError.value) ? "正在加载台账…" : loadError.value ? "台账加载失败，请重新加载" : "当前筛选条件下暂无台账数据");
...
async function exportReport(table: HTMLTableElement): Promise<void> {
  ...
  try { await exportLedgerTable(table, ledgerExportName("街道台账", report.value.query)); }
  catch (error) { ElMessage.error(errorMessage(error, "台账导出失败，请重试")); }
...
<template>
  <LedgerReportFrame title="街道台账" :loading="loading" :export-disabled="!canExport" :exporting="exporting" @refresh="load" @export="exportReport">
