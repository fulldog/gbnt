...
// server.mode=debug|dev 时每次启动 DROP 当前库全部表再按模型重建并写入组织、管理员角色、API 目录与 admin 用户；release 仅 AutoMigrate + 同步 API + 可选空库种子。
package migrate
...
type Options struct {
	...
	Dev  bool // debug/dev：DROP 全部表后重建并写入组织、管理员角色、API、admin 用户
}
