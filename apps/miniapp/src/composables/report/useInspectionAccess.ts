...
/** 巡查入口检查定位、相机及微信系统权限；拒绝后保留草稿并等待用户打开设置。 */
export function useInspectionAccess(options: InspectionAccessOptions = {}) {
  const phase = shallowRef<"idle" | "checking" | "denied" | "ready">("idle");
  ...
  async function check(): Promise<void> {
    ...
    try {
      ...
      phase.value = "denied";
      message.value = /privacy|requiredPrivateInfos/i.test(error.errMsg || "")
        ? deviceFailureMessage(error, "继续巡查")
        : error.message || "巡查需要定位和相机权限，请授权并开启手机定位后重试。";
    }
  }
  ...
  return { phase, message, position, ready, request, openSettings, denyMedia };
}
